...
function submitCourses($year, $term, &$courses)
{
	try
	{
		...
		foreach($courses as $key=>$value)
		{
			...
			/*
			$stmt = $db->prepare("SELECT course_info_id FROM Course_Info
									WHERE dept_name LIKE :dept_name AND course_num LIKE :course_num
									AND credits = :credits AND course_details LIKE :course_details");
						
			$stmt->bindValue(':dept_name', '%' . $value['dept_name'] . '%');
			$stmt->bindValue(':course_num', '%' . $value['course_num'] . '%');
			$stmt->bindValue(':credits', intval($value['credits']));
			...
			*/
			...
			foreach($result4 as $row4)
			{
				...
				array_push($courses[$key]['assign'], $ta_assign_name);
			}
		}
