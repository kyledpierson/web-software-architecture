...
<?php
  ...
  require_once "Models/App_Courses.php";
  verifyLogin(0);
  session_start();
  
  $app_courses = new App_Courses();
?>
...
<script src="Models/app_functions.js"></script>
  ...
  <h2>Please fill out the form below</h2>
	...
	<form name="app_form" method="post" action="Models/process.php" enctype="multipart/form-data" onsubmit="return validate();">
	  ...
	  <p>
      <input type="radio" name="Academic" id="AcademicFresh" value="Freshman" checked="checked"/> 
      <label for="Academic">Freshman</label>
	  <br>
      
	  <input type="radio" name="Academic" id="AcademicSoph" value="Sophomore" /> 
      <label for="Academic">Sophomore</label>
	  <br>
	  
	  <input type="radio" name="Academic" id="AcademicJun" value="Junior" /> 
      <label for="Academic">Junior</label>
	  <br>
      
	  <input type="radio" name="Academic" id="AcademicSen" value="Senior" /> 
      <label for="Academic">Senior</label>
	  <br>
	  
	  <input type="radio" name="Academic" id="AcademicGrad" value="Graduate" /> 
      ...
      <label id="lbl">Only entries in which all fields have been selected will be recorded</label><br>
      <label id="lbl">Make sure that the course term and year are correct, or the course may not be recorded</label>
      <table id="Class_List">
        <tr class="block">
        	...
        	<select name="loop[]" class="class">
				...
				<?php
                foreach($app_courses->classes as $value)
				{echo "<option value='" . substr($value, strpos($value, ' ') + 1) . "'>" . $value . "</option>";}
				?>
			</select>
